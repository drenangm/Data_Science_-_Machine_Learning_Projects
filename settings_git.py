...
def get_layout():
    try:
        ...
        # Dataframe
        config_df = data_operations_git.read_config_in_df()
                    ...
                    dbc.Row([
                        ...
                        dbc.Col([
                            dbc.Input(id = "input_dateformat",
                                ...
                                value = constant_git.DATE_FORMAT
                            )
                        ], width=3)
                    ], no_gutters=True),
                    dbc.Row([
...
# Callback
@app_git.callback(Output(component_id='example-output', component_property='children'), 
...
def on_button_click(num_click, table_data, dt_format):
    if num_click > 0:
        if data_operations_git.write_field_mapping_file(table_data,dt_format) == 0:
            config_df = data_operations_git.read_config_in_df()
