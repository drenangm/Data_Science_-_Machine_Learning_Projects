...
from modulos import data_operations_A, constant_A
...
def get_layout():
    try:
        ...
        # Dataframe
        config_df = data_operations_A.read_config_in_df()
...
# Callback
@app_A.callback(Output(component_id='example-output', component_property='children'), 
...
def on_button_click(num_click, table_data, dt_format):
    if num_click > 0:
        if data_operations_A.write_field_mapping_file(table_data,dt_format) == 0:
            config_df = data_operations_A.read_config_in_df()
